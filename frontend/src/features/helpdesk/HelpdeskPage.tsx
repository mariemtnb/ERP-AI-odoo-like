...
export default function HelpdeskPage() {
  ...
  return (
    <div>
      <PageHead title={tr("nav.helpdesk")} sub={tr("hd.sub")}>
        <Button onClick={() => setCreating((v) => !v)}>{creating ? tr("common.close") : tr("hd.new")}</Button>
      </PageHead>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
        {["", "open", "in_progress", "resolved", "closed"].map((s) => (
          ...
          }}>{s ? tr(`hd.status.${s}`) : tr("common.all")}</button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 18, alignItems: "start" }}>
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
          {(ticketsQ.data ?? []).map((tk) => (
            <button key={tk.id} onClick={() => setSelected(tk.id)} style={{
              display: "block", width: "100%", textAlign: "left", padding: "12px 14px", cursor: "pointer",
              background: selected === tk.id ? "color-mix(in oklab, var(--emerald-500) 12%, transparent)" : "transparent",
              border: 0, borderBottom: "1px solid var(--border)", color: "var(--text-strong)",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                <span style={{ fontWeight: 600 }}>{tk.subject}</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: PRIORITY_COLOR[tk.priority] }}>{tr(`hd.priority.${tk.priority}`)}</span>
              </div>
              <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{tk.number} · {tr(`hd.status.${tk.status}`)} · {tk.messages_count} {tr("hd.msg")}</div>
            </button>
          ))}
          {ticketsQ.data?.length === 0 && <p style={{ padding: 14, color: "var(--text-muted)", fontSize: 13 }}>{tr("hd.none")}</p>}
        </div>

        <div>{selected ? <TicketDetail ticketId={selected} /> : <p style={{ color: "var(--text-muted)" }}>{tr("hd.select")}</p>}</div>
      </div>
    </div>
  );
}
...
function TicketDetail({ ticketId }: { ticketId: number }) {
  ...
  const t = q.data;
  if (!t) return <p style={{ color: "var(--text-muted)" }}>{tr("common.loading")}</p>;
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
      <div style={{ padding: 16, borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{t.number} · {t.customer_name ?? tr("hd.noCustomer")} · {tr("hd.priorityWord")} <span style={{ color: PRIORITY_COLOR[t.priority] }}>{tr(`hd.priority.${t.priority}`)}</span></div>
          </div>
          <span style={{ fontSize: 12, fontWeight: 600, color: "var(--emerald-400)", border: "1px solid var(--emerald-500)", borderRadius: 999, padding: "3px 12px" }}>{tr(`hd.status.${t.status}`)}</span>
        </div>
        {isManager && NEXT[t.status].length > 0 && (
          <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
            {NEXT[t.status].map((s) => (
              <Button key={s} size="sm" variant="outline" loading={setStatus.isPending} onClick={() => setStatus.mutate(s)}>{tr("hd.mark")} {tr(`hd.status.${s}`).toLowerCase()}</Button>
            ))}
          </div>
        )}
      </div>
      ...
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12, maxHeight: 380, overflowY: "auto" }}>
        ...
        {t.messages.length === 0 && <p style={{ color: "var(--text-muted)", fontSize: 13 }}>{tr("hd.noMessages")}</p>}
      </div>

      {t.status !== "closed" && (
        <div style={{ display: "flex", gap: 8, padding: 14, borderTop: "1px solid var(--border)" }}>
          <Input placeholder={tr("hd.writeReply")} value={body} onChange={(e) => setBody(e.target.value)} style={{ flex: 1 }}
            onKeyDown={(e) => { if (e.key === "Enter" && body.trim()) reply.mutate(); }} />
          <Button loading={reply.isPending} disabled={!body.trim()} onClick={() => reply.mutate()}>{tr("hd.send")}</Button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18, marginBottom: 18 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12, marginBottom: 12 }}>
        <Field label={tr("hd.subject")}><Input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder={tr("hd.subjectPlaceholder")} /></Field>
        <Field label={tr("hd.priorityLabel")}>
          <select value={priority} onChange={(e) => setPriority(e.target.value)} style={{ width: "100%", height: 38, background: "var(--surface-hover)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-strong)", padding: "0 8px" }}>
            <option value="low">{tr("hd.priority.low")}</option><option value="normal">{tr("hd.priority.normal")}</option><option value="high">{tr("hd.priority.high")}</option><option value="urgent">{tr("hd.priority.urgent")}</option>
          </select>
        </Field>
      </div>
      <Field label={tr("hd.firstMessage")}><Input value={message} onChange={(e) => setMessage(e.target.value)} placeholder={tr("hd.firstMsgPlaceholder")} /></Field>
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        <Button variant="outline" onClick={onCancel}>{tr("common.cancel")}</Button>
        <Button loading={create.isPending} disabled={!subject.trim()} onClick={() => create.mutate()}>{tr("hd.createBtn")}</Button>
      </div>
    </div>
  );
}

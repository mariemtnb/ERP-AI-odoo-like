...
export default function ManufacturingPage() {
  ...
  return (
    <div>
      <PageHead title="Manufacturing" sub="Bills of materials and work orders that consume components and produce finished goods." />
      <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
        {(["orders", "boms"] as Tab[]).map((t) => (
          ...
          }}>{t === "orders" ? "Work orders" : "Bills of materials"}</button>
        ))}
      </div>
...
function WorkOrders() {
  ...
  const create = useMutation({
    ...
    onError: (e: any) => setError(e?.response?.data?.detail ?? "Could not create the work order."),
  });
  const act = useMutation({
    mutationFn: ({ id, a }: { id: number; a: "start" | "complete" | "cancel" }) => mfg.workOrderAction(id, a),
    onSuccess: () => { setError(null); refresh(); },
    onError: (e: any) => setError(e?.response?.data?.detail ?? "Action failed."),
  });
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr auto", gap: 10, alignItems: "end" }}>
          <Field label="Bill of materials">
            <select value={bom} onChange={(e) => setBom(e.target.value)} style={selectStyle}>
              ...
              {(bomsQ.data ?? []).map((b) => <option key={b.id} value={b.id}>{b.product_name} (batch of {b.output_quantity})</option>)}
            </select>
          </Field>
          <Field label="Quantity to produce"><Input type="number" min={0} step="0.001" value={qty} onChange={(e) => setQty(e.target.value)} /></Field>
          <Button loading={create.isPending} disabled={!bom || !(Number(qty) > 0)} onClick={() => create.mutate()}>Create work order</Button>
        </div>
          ...
          <tr key={w.id} style={rowStyle}>
            ...
            <Td right>
              <span style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                {w.status === "draft" && <>
                  <Button size="sm" variant="outline" onClick={() => act.mutate({ id: w.id, a: "start" })}>Start</Button>
                  <Button size="sm" onClick={() => act.mutate({ id: w.id, a: "complete" })}>Complete</Button>
                  <Button size="sm" variant="ghost" onClick={() => act.mutate({ id: w.id, a: "cancel" })}>Cancel</Button>
                </>}
                {w.status === "in_progress" && <Button size="sm" onClick={() => act.mutate({ id: w.id, a: "complete" })}>Complete</Button>}
              </span>
            </Td>
          </tr>
        ))}
        {wosQ.data?.length === 0 && <tr><Td colSpan={5} muted>No work orders yet.</Td></tr>}
      </Table>
    </>
  );
}
  ...
  return (
    <>
      <div style={{ marginBottom: 14 }}>
        <Button onClick={() => setCreating((v) => !v)} variant="outline">{creating ? "Close" : "New BOM"}</Button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {(bomsQ.data ?? []).map((b) => (
          <div key={b.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
            <div style={{ fontWeight: 700, color: "var(--text-strong)" }}>{b.product_name} <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>· batch of {b.output_quantity}</span></div>
              ...
              <tbody>
                {b.components.map((c) => (
                  <tr key={c.component}><td style={{ padding: "4px 0", color: "var(--text-body)" }}>{c.component_name}</td>
                    <td style={{ padding: "4px 0", textAlign: "right", fontFamily: "var(--font-mono)", color: "var(--text-muted)" }}>{c.quantity} per batch · {c.in_stock} in stock</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ))}
        {bomsQ.data?.length === 0 && <p style={{ color: "var(--text-muted)" }}>No bills of materials yet.</p>}
      </div>
    </>
  );
}
...
function NewBom({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const add = useMutation({
    ...
    onSuccess: onDone,
    onError: (e: any) => setError(e?.response?.data?.detail ?? "Could not create the BOM (one per product)."),
  });
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18, marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <strong style={{ color: "var(--text-strong)" }}>New bill of materials</strong>
        <Button variant="ghost" size="sm" onClick={onCancel}>Cancel</Button>
      </div>
      {!product ? (
        <>
          <Field label="Finished product"><Input placeholder="Search product…" value={search} onChange={(e) => setSearch(e.target.value)} /></Field>
        ...
        <>
          <div style={{ display: "flex", gap: 12, alignItems: "end", marginBottom: 12 }}>
            <div style={{ color: "var(--text-strong)" }}>{product.name} <button onClick={() => setProduct(null)} style={{ color: "var(--emerald-400)", background: "none", border: 0, cursor: "pointer", fontSize: 12 }}>change</button></div>
            <Field label="Units per batch"><Input type="number" min={1} step="0.001" value={output} onChange={(e) => setOutput(e.target.value)} style={{ width: 120 }} /></Field>
          </div>
          <div style={{ marginBottom: 8, color: "var(--text-muted)", fontSize: 13 }}>Components</div>
          {rows.map((r, i) => (
            <div key={r.product.id} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6 }}>
              ...
              <input type="number" min={0} step="0.001" value={r.qty} onChange={(e) => setRows((rs) => rs.map((x, j) => j === i ? { ...x, qty: e.target.value } : x))}
                placeholder="qty/batch" style={{ width: 110, background: "var(--surface-hover)", border: "1px solid var(--border)", borderRadius: 8, padding: "6px 8px", color: "var(--text-strong)" }} />
              <button onClick={() => setRows((rs) => rs.filter((_, j) => j !== i))} style={{ color: "var(--rose-400)", background: "none", border: 0, cursor: "pointer" }}>×</button>
            </div>
          ))}
          <Input placeholder="Add a component…" value={compSearch} onChange={(e) => setCompSearch(e.target.value)} style={{ marginTop: 6 }} />
          ...
          <div style={{ marginTop: 14 }}>
            <Button loading={add.isPending} disabled={!ok} onClick={() => add.mutate()}>Create BOM</Button>
          </div>
        </>
      )}
      {error && <p style={{ color: "var(--rose-400)", fontSize: 13, marginTop: 10 }}>{error}</p>}
...
function Badge({ status }: { status: string }) {
  ...
  return <span style={{ textTransform: "capitalize", fontSize: 12, fontWeight: 600, color: c, border: `1px solid ${c}`, borderRadius: 999, padding: "2px 10px" }}>{status.replace("_", " ")}</span>;
}

...
export default function RfqPage() {
  ...
  return (
    <div>
      <PageHead title="Requests for Quotation" sub="Ask suppliers to bid, compare their totals, and award the winner as a draft purchase order.">
        <Button onClick={() => setCreating((v) => !v)}>{creating ? "Close" : "New RFQ"}</Button>
      </PageHead>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
          {(rfqsQ.data ?? []).map((r) => (
            <button key={r.id} onClick={() => setSelected(r.id)} style={{
              ...
              <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{r.bids_count} bids · {r.status}</div>
            </button>
          ))}
          {rfqsQ.data?.length === 0 && <p style={{ padding: 14, color: "var(--text-muted)", fontSize: 13 }}>No RFQs yet.</p>}
        </div>

        <div>{selected ? <RfqDetail rfqId={selected} /> : <p style={{ color: "var(--text-muted)" }}>Select an RFQ.</p>}</div>
      </div>
    </div>
  );
}
...
function RfqDetail({ rfqId }: { rfqId: number }) {
  ...
  const rfq = q.data;
  if (!rfq) return <p style={{ color: "var(--text-muted)" }}>Loading…</p>;
    ...
    <>
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 16, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <strong style={{ color: "var(--text-strong)" }}>{rfq.title} — requested items</strong>
          {open && <Button size="sm" variant="outline" onClick={() => setBidding((v) => !v)}>{bidding ? "Close" : "Enter a bid"}</Button>}
        </div>
          ...
          <tbody>
            {rfq.lines.map((l) => (
              <tr key={l.id}><td style={{ padding: "4px 0", color: "var(--text-body)" }}>{l.product_name} <span style={{ color: "var(--text-muted)" }}>· {l.sku}</span></td>
                <td style={{ padding: "4px 0", textAlign: "right", fontFamily: "var(--font-mono)", color: "var(--text-muted)" }}>qty {l.quantity}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <h3 style={{ font: "600 15px var(--font-sans)", color: "var(--text-strong)", margin: "4px 0 10px" }}>Bid comparison</h3>
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          <thead><tr style={{ background: "var(--surface-hover)", color: "var(--text-muted)", textAlign: "left" }}>
            <Th>Supplier</Th><Th right>Total</Th><Th>Status</Th><Th></Th>
          </tr></thead>
          <tbody>
            {rfq.comparison.map((b) => (
              <tr key={b.id} style={{ borderTop: "1px solid var(--border)", background: b.is_lowest ? "color-mix(in oklab, var(--emerald-500) 8%, transparent)" : undefined }}>
                <Td>{b.supplier_name} {b.is_lowest && <span style={{ marginLeft: 8, fontSize: 11, color: "var(--emerald-400)", border: "1px solid var(--emerald-500)", borderRadius: 999, padding: "1px 7px" }}>lowest</span>}</Td>
                <Td right mono style={{ fontWeight: 600, color: "var(--text-strong)" }}>{money(b.total_amount)} TND</Td>
                <Td><span style={{ textTransform: "capitalize", fontSize: 12, fontWeight: 600, color: b.status === "awarded" ? "var(--emerald-400)" : b.status === "rejected" ? "var(--rose-400)" : "var(--text-muted)" }}>{b.status}</span></Td>
                <Td right>{open && <Button size="sm" loading={award.isPending} onClick={() => award.mutate(b.id)}>Award</Button>}</Td>
              </tr>
            ))}
            {rfq.comparison.length === 0 && <tr><Td colSpan={4} muted>No bids yet.</Td></tr>}
          </tbody>
        </table>
      </div>
    </>
...
function BidForm({ rfq, onDone }: { rfq: rfqApi.Rfq; onDone: () => void }) {
  ...
  const submit = useMutation({
    ...
    onSuccess: onDone,
    onError: (e: any) => setError(e?.response?.data?.detail ?? "Could not submit the bid."),
  });
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18, marginBottom: 16 }}>
      <div style={{ marginBottom: 12, maxWidth: 300 }}>
        <span style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Supplier</span>
      ...
      <table style={{ width: "100%", fontSize: 14, borderCollapse: "collapse" }}>
        <thead><tr style={{ color: "var(--text-muted)", textAlign: "left" }}><Th>Item</Th><Th right>Qty</Th><Th right>Unit price</Th></tr></thead>
        <tbody>
          {rfq.lines.map((l) => (
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 12 }}>
        ...
        <Button loading={submit.isPending} disabled={!supplier || !allPriced} onClick={() => submit.mutate()}>Submit bid</Button>
      </div>
...
function NewRfq({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const add = useMutation({
    ...
    onSuccess: onDone,
    onError: (e: any) => setError(e?.response?.data?.detail ?? "Could not create the RFQ."),
  });
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18, marginBottom: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <strong style={{ color: "var(--text-strong)" }}>New request for quotation</strong>
        <Button variant="ghost" size="sm" onClick={onCancel}>Cancel</Button>
      </div>
      <Input placeholder="Title (e.g. Q3 beverage restock)" value={title} onChange={(e) => setTitle(e.target.value)} />
      <div style={{ margin: "12px 0 6px", color: "var(--text-muted)", fontSize: 13 }}>Items</div>
      {rows.map((r, i) => (
        <div key={r.product.id} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6 }}>
          ...
          <input type="number" min={0} step="0.001" value={r.qty} onChange={(e) => setRows((rs) => rs.map((x, j) => j === i ? { ...x, qty: e.target.value } : x))}
            placeholder="qty" style={{ width: 100, background: "var(--surface-hover)", border: "1px solid var(--border)", borderRadius: 8, padding: "6px 8px", color: "var(--text-strong)" }} />
          <button onClick={() => setRows((rs) => rs.filter((_, j) => j !== i))} style={{ color: "var(--rose-400)", background: "none", border: 0, cursor: "pointer" }}>×</button>
        </div>
      ))}
      <Input placeholder="Add a product…" value={search} onChange={(e) => setSearch(e.target.value)} style={{ marginTop: 6 }} />
      ...
      <div style={{ marginTop: 14 }}><Button loading={add.isPending} disabled={!ok} onClick={() => add.mutate()}>Create RFQ</Button></div>
      {error && <p style={{ color: "var(--rose-400)", fontSize: 13, marginTop: 8 }}>{error}</p>}
    </div>
  );
}
